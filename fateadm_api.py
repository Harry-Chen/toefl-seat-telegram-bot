...
def CalcSign(pd_id, passwd, timestamp):
    md5     = hashlib.md5()
    md5.update(timestamp.encode() + passwd.encode())
    csign   = md5.hexdigest()

    md5     = hashlib.md5()
    md5.update(pd_id.encode() + timestamp.encode() + csign.encode())
    csign   = md5.hexdigest()
    return csign

def CalcCardSign(cardid, cardkey, timestamp, passwd):
    md5     = hashlib.md5()
    md5.update(passwd.encode() + timestamp.encode() + cardid.encode() + cardkey.encode())
    return md5.hexdigest()

def HttpRequest(url, body_data):
    rsp         = Rsp()
    r = requests.post(url, data=body_data, headers={'user-agent': 'Mozilla/5.0'})
